...
describe("ERC20 mock contract", function () {
    ...
    describe("Transactions", function () {
        it("Should transfer tokens between accounts", async function () {
            ...
            await this.token2.connect(this.bob).transfer(this.alice.address, transferAmount)

            const aliceBalance = await this.token2.balanceOf(this.alice.address)
        ...
        it("Should update balances after transfers", async function () {
            const initialOwnerBalance = await this.token2.balanceOf(this.bob.address)

            const transferToMishaAmount = ethers.utils.parseUnits("100", 6)
            await this.token2.connect(this.bob).transfer(this.misha.address, transferToMishaAmount)

            const transferToTemaAmount = ethers.utils.parseUnits("100", 6)
            await this.token2.connect(this.bob).transfer(this.tema.address, transferToTemaAmount)

            const finalOwnerBalance = await this.token2.balanceOf(this.bob.address)
            expect(finalOwnerBalance).to.equal(initialOwnerBalance.sub(transferToTemaAmount).sub(transferToMishaAmount))

            const mishaBalance = await this.token2.balanceOf(this.misha.address)
            expect(mishaBalance).to.equal(transferToMishaAmount)

            const temaBalance = await this.token2.balanceOf(this.tema.address)
